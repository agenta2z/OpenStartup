...
def _build_headers(tenant_id: str | None = None) -> dict[str, str]:
    ...
    auth_token = os.environ.get("PROMETHEUS_AUTH_TOKEN") or os.environ.get(
        "MIMIR_AUTH_TOKEN"
    )
    if auth_token:
        ...
        if action == "instant_query":
            ...
            if not query:
                return {
                    ...
                    "error": "Parameter 'query' is required for instant_query.",
                }
        ...
        # --- Range Query ---
            ...
            if not query:
                return {
                    "status": "error",
                    "errorType": "missing_parameter",
                    "error": "Parameter 'query' is required for range_query.",
                }
            if not start or not end:
                return {
                    "status": "error",
                    "errorType": "missing_parameter",
                    "error": "Parameters 'start' and 'end' are required for range_query.",
                }
        ...
        # --- Series Discovery ---
            ...
            if not match_selector:
                return {
                    "status": "error",
                    "errorType": "missing_parameter",
                    "error": "Parameter 'match' is required for list_series.",
                }
        ...
        # --- Label Values ---
            ...
            if not label_name:
                return {
                    "status": "error",
                    "errorType": "missing_parameter",
                    "error": "Parameter 'label_name' is required for label_values.",
                }
