...
async def _create_alert(
    session: aiohttp.ClientSession, base_url: str, headers: dict, args: dict
) -> dict:
    ...
    async with session.post(
        f"{base_url}/v2/alerts", headers=headers, json=body
    ) as resp:
        data = await resp.json()
        if resp.status in (200, 201, 202):
            return _ok(
                data, "Alert creation request accepted", data.get("requestId", "")
            )
...
async def _get_alert(
    ...
    async with session.get(
        f"{base_url}/v2/alerts/{alert_id}", headers=headers, params=params
    ) as resp:
        data = await resp.json()
        if resp.status == 200:
...
async def _list_alerts(
    ...
    async with session.get(
        f"{base_url}/v2/alerts", headers=headers, params=params
    ) as resp:
        data = await resp.json()
        if resp.status == 200:
...
async def _count_alerts(
    ...
    async with session.get(
        f"{base_url}/v2/alerts/count", headers=headers, params=params
    ) as resp:
        data = await resp.json()
        if resp.status == 200:
...
async def _alert_action(
    ...
    async with session.post(
        ...
        if resp.status in (200, 202):
            return _ok(
                data, f"Alert {action} request accepted", data.get("requestId", "")
            )
...
async def _add_note(
    ...
    async with session.post(
        f"{base_url}/v2/alerts/{alert_id}/notes",
        headers=headers,
        params=params,
        json=body,
    ) as resp:
...
async def _add_tags(
    ...
    async with session.post(
        f"{base_url}/v2/alerts/{alert_id}/tags",
        headers=headers,
        params=params,
        json=body,
    ) as resp:
...
async def _update_priority(
    ...
    async with session.put(
        f"{base_url}/v2/alerts/{alert_id}/priority",
        headers=headers,
        params=params,
        json=body,
    ) as resp:
        ...
        if resp.status in (200, 202):
            return _ok(
                data,
                f"Priority updated to {args['priority']}",
                data.get("requestId", ""),
            )
...
async def _create_incident(
    ...
    async with session.post(
        f"{base_url}/v1/incidents/create", headers=headers, json=body
    ) as resp:
        data = await resp.json()
        if resp.status in (200, 201, 202):
            return _ok(
                data, "Incident creation request accepted", data.get("requestId", "")
            )
...
async def _get_incident(
    ...
    async with session.get(
        f"{base_url}/v1/incidents/{incident_id}", headers=headers
    ) as resp:
        data = await resp.json()
        if resp.status == 200:
...
async def _list_incidents(
    ...
    async with session.get(
        f"{base_url}/v1/incidents", headers=headers, params=params
    ) as resp:
        data = await resp.json()
        if resp.status == 200:
...
async def _close_incident(
    ...
    async with session.post(
        f"{base_url}/v1/incidents/{incident_id}/close", headers=headers, json=body
    ) as resp:
        data = await resp.json()
        if resp.status in (200, 202):
            return _ok(
                data, "Incident close request accepted", data.get("requestId", "")
            )
        return _err(f"Close incident failed: {resp.status}", data)


async def _add_responder(
    session: aiohttp.ClientSession, base_url: str, headers: dict, args: dict
) -> dict:
    incident_id = args.get("incident_id")
    if not incident_id:
    ...
    async with session.post(
        f"{base_url}/v1/incidents/{incident_id}/responders", headers=headers, json=body
    ) as resp:
        data = await resp.json()
        if resp.status in (200, 202):
...
async def _add_timeline_entry(
    ...
    async with session.post(
        f"{base_url}/v1/incidents/{incident_id}/timeline", headers=headers, json=body
    ) as resp:
        data = await resp.json()
        if resp.status in (200, 202):
...
async def _get_on_call(
    ...
    async with session.get(
        f"{base_url}/v2/schedules/{schedule_id}/on-calls", headers=headers
    ) as resp:
        data = await resp.json()
        if resp.status == 200:
...
async def _get_escalation_policy(
    ...
    async with session.get(
        f"{base_url}/v2/escalations/{esc_id}", headers=headers
    ) as resp:
        data = await resp.json()
        if resp.status == 200:
...
async def _list_escalation_policies(
    session: aiohttp.ClientSession, base_url: str, headers: dict, args: dict
) -> dict:
...
_ACTION_DISPATCH = {
    ...
    "unacknowledge_alert": lambda s, b, h, a: _alert_action(
        s, b, h, a, "unacknowledge"
    ),
...
async def execute(
    ...
    if action not in _ACTION_DISPATCH:
        return _err(
            f"Unknown action: {action}. Valid actions: {sorted(_ACTION_DISPATCH.keys())}"
        )
