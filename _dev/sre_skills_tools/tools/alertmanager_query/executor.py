...
async def _list_alerts(
    session: aiohttp.ClientSession, base_url: str, args: dict
) -> dict:
    ...
    async with session.get(f"{base_url}{_API_BASE}/alerts", params=params) as resp:
        if resp.status == 200:
            ...
            return _ok(
                truncated,
                f"Retrieved {len(truncated)} alerts (total: {len(alerts)})",
                count=len(alerts),
            )
...
async def _list_alert_groups(
    ...
    async with session.get(
        f"{base_url}{_API_BASE}/alerts/groups", params=params
    ) as resp:
        if resp.status == 200:
            groups = await resp.json()
            return _ok(
                groups, f"Retrieved {len(groups)} alert groups", count=len(groups)
            )
        text = await resp.text()
        return _err(f"List alert groups failed: {resp.status} — {text}")


async def _list_silences(
    session: aiohttp.ClientSession, base_url: str, args: dict
) -> dict:
    params: list[tuple[str, str]] = []
    if args.get("filter"):
    ...
    async with session.get(f"{base_url}{_API_BASE}/silences", params=params) as resp:
        if resp.status == 200:
            ...
            return _ok(
                truncated,
                f"Retrieved {len(truncated)} silences (total: {len(silences)})",
                count=len(silences),
            )
        text = await resp.text()
...
async def _create_silence(
    ...
    if not args.get("matchers"):
        return _err(
            "--matchers is required for create_silence (JSON array of matcher objects)"
        )
...
async def _list_receivers(
    ...
    async with session.get(f"{base_url}{_API_BASE}/receivers") as resp:
        if resp.status == 200:
            ...
            return _ok(
                receivers, f"Retrieved {len(receivers)} receivers", count=len(receivers)
            )
        text = await resp.text()
