...
def _ok(
    data: Any, metadata: dict | None = None, warnings: list[str] | None = None
) -> dict[str, Any]:
    return {
        "ok": True,
        "data": data,
...
def _err(
    message: str, *, provider: str | None = None, action: str | None = None
) -> dict[str, Any]:
    out: dict[str, Any] = {
        "ok": False,
        "error": message,
...
def _resolve_time_range(arguments: dict[str, Any]) -> tuple[float, float]:
    """Returns (start_epoch_sec, end_epoch_sec)."""
        ...
        _parse_timestamp(arguments.get("end")) if arguments.get("end") else time.time()
    )
...
def _get_auth_kwargs() -> dict:
    if (u := os.environ.get("LOG_QUERY_USER")) and (
        p := os.environ.get("LOG_QUERY_PASSWORD")
    ):
...
class LokiProvider:
        ...
        try:
            async with session.get(url, params=params, headers=self.headers) as resp:
                ...
                try:
                    ...
                    return _err(
                        f"Loki returned non-JSON: {text[:200]}", provider="loki"
                    )
    ...
    def _normalize_streams(loki_data: dict) -> dict:
        ...
        if result_type == "streams":
            streams = [
                {
                    "stream": s.get("stream", {}),
                    "values": [[_to_iso(v[0]), v[1]] for v in s.get("values", [])],
                }
                for s in result
            ]
    ...
    async def query(self, session: aiohttp.ClientSession, arguments: dict) -> dict:
        ...
        if not q:
            return _err(
                "--query is required for query action", provider="loki", action="query"
            )
        params: dict[str, Any] = {
            "query": q,
            "limit": str(
                min(int(arguments.get("limit") or _DEFAULT_LIMIT), _LOKI_HARD_LIMIT)
            ),
    ...
    async def query_range(
        self, session: aiohttp.ClientSession, arguments: dict
    ) -> dict:
        q = arguments.get("query")
        if not q:
            return _err(
                "--query is required for query_range",
                provider="loki",
                action="query_range",
            )
        try:
        ...
        for stream in normalized.get("streams", []):
            if len(stream.get("values", [])) >= int(params["limit"]):
                warnings.append(
                    f"Result truncated at limit={params['limit']}; consider narrower query or increase --limit"
                )
                break
        return _ok(
            normalized,
    ...
    async def volume(self, session: aiohttp.ClientSession, arguments: dict) -> dict:
        ...
        if not q:
            return _err(
                "--query is required for volume", provider="loki", action="volume"
            )
        try:
            start, end = _resolve_time_range(arguments)
        except ValueError as e:
...
async def execute(
    ...
    if handler is None:
        return _err(
            f"Action {action!r} not implemented on {provider_name!r}", action=action
        )
    ...
    async with aiohttp.ClientSession(**session_kwargs) as session:
        try:
            ...
            logger.exception(
                "log_query.execute crashed in %s.%s", provider_name, action
            )
            return _err(
                f"Unexpected error: {exc!r}", provider=provider_name, action=action
            )
