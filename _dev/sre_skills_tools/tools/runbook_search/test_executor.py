...
class TestResolveProvider:
    def test_explicit_wins(self):
        assert (
            executor._resolve_provider(
                {"provider": "notion"}, "https://x.atlassian.net/wiki"
            )
            == "notion"
        )
    ...
    def test_env_overrides_url(self, monkeypatch):
        ...
        assert (
            executor._resolve_provider({}, "https://x.atlassian.net/wiki") == "mkdocs"
        )

    def test_url_heuristic_confluence(self):
        assert (
            executor._resolve_provider({}, "https://x.atlassian.net/wiki")
            == "confluence"
        )
...
class TestResolveEndpoint:
    def test_explicit_arg(self, monkeypatch):
        ...
        assert (
            executor._resolve_endpoint({"endpoint": "http://arg.url"})
            == "http://arg.url"
        )
...
class TestExecuteValidation:
    ...
    def test_no_endpoint_for_confluence(self, monkeypatch):
        ...
        result = _run(
            executor.execute(
                {"action": "search", "query": "x", "provider": "confluence"}
            )
        )
        assert not result["ok"]
                    ...
                    "results": [
                        {
                            "content": {
                                ...
                                "_links": {
                                    "webui": "/spaces/ENG/pages/1234/On-call+Runbook"
                                },
                            },
    ...
    def test_search_emits_truncation_warning(self, monkeypatch):
        async def fake_request(self, session, path, params=None):
            return {
                "_raw": {
                    "results": [
                        {"content": {"id": str(i), "title": f"p{i}"}} for i in range(5)
                    ],
                    "totalSize": 100,
                }
            }
                    ...
                    "results": [
                        {
                            "content": {
                                ...
                                "title": "T",
                                "body": {
                                    "storage": {"value": "<p>full body content</p>"}
                                },
                            },
                            "excerpt": "snip",
                        }
    ...
    def test_http_error_returned_cleanly(self, monkeypatch):
        async def fake_request(self, session, path, params=None):
            return executor._err(
                "Confluence HTTP 401: unauthorized", provider="confluence"
            )
...
class TestStubProviders:
    def test_atlassian_docs_stub(self):
        result = _run(
            executor.execute(
                {"action": "search", "query": "x", "provider": "atlassian_docs"}
            )
        )
        assert not result["ok"]
        assert "atlassian_docs" in result["error"].lower()
...
class TestResultShape:
    ...
    def test_failure_shape(self):
        result = _run(
            executor.execute({"action": "search"})
        )  # no endpoint, default atlassian_docs stub
