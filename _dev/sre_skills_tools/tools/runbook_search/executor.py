...
class ConfluenceProvider:
    ...
    def _normalize_search_result(
        raw: dict, base_url: str, include_body: bool
    ) -> list[dict]:
        out: list[dict] = []
        for entry in raw.get("results", []):
            ...
            labels_raw = ((content.get("metadata") or {}).get("labels") or {}).get(
                "results", []
            )
            ...
            last_updated = (
                content.get("version") or entry.get("lastModified") or {}
            ).get("when")
            row = {
                "id": page_id,
                "title": title,
            ...
            if include_body and content.get("body", {}).get("storage", {}).get("value"):
                row["body"] = _strip_html(
                    content["body"]["storage"]["value"], max_len=10_000
                )
            out.append(row)
        return out
    ...
    async def search(self, session: aiohttp.ClientSession, arguments: dict) -> dict:
        ...
        rows = self._normalize_search_result(
            raw["_raw"], self.endpoint, bool(arguments.get("include_body"))
        )
        warnings: list[str] = []
        total = raw["_raw"].get("totalSize") or raw["_raw"].get("size", len(rows))
        if total > limit:
            warnings.append(
                f"Truncated: total={total} returned={limit}; pass --limit to widen"
            )
        return _ok(
            {"results": rows},
            metadata={
    ...
    async def list_by_label(
        ...
        if not arguments.get("label"):
            return _err(
                "--label is required for list_by_label",
                provider=self.name,
...
def _get_auth_kwargs() -> dict:
    if (u := os.environ.get("CONFLUENCE_USER")) and (
...
async def execute(
    ...
    async with aiohttp.ClientSession(**session_kwargs) as session:
        try:
            ...
            logger.exception(
                "runbook_search.execute crashed in %s.%s", provider_name, action
            )
            return _err(
