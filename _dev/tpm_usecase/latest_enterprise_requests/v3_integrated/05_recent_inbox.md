# 05 · Recent ENT Inbox (last 60 days)
*(verified 2026-05-15)*

> All metadata is **verified live** against `hello.atlassian.net` Jira on 2026-05-15. Source: `data/batchA.json` (46 tickets, ENT-3884 → ENT-3837) + `data/batchB.json` (57 tickets including referenced legacy items). The "Pillar route" column is what a TPM should use to forward each item.
...
| Tickets with no assignee | **7** (need triage — see §C) |

## A — High-priority recent items (Blocker / Critical / Major)

| Key | Pri | Assignee | Pillar route | Component | Why it matters |
|---|---|---|---|---|---|
...
## B — Pillar-routed inbox (Minor priority bulk)

> See [`03_master_mapping.md`](03_master_mapping.md) §C for full per-row tables. Summary buckets only here.

* **→ Identity (David Dooley / Dushyant Gill):** ENT-3869, 3867, 3848, 3824, 3811, 3810, 3855, 3837, 3789, 3746, 3745, 3738, 3737, 3736, 3730, 3782, 3784, 3739
* **→ TSP / TDP (Corey Johnston / Vinod Kumar):** ENT-3790, 3788, 3787, 3785, 3668, 3812
* **→ AI Platform (Rovo) — outside CoreEng but routed via MUSTWIN:** ENT-3879, 3866, 3865, 3864, 3860, 3856, 3849, 3843, 3842, 3841, 3838, 3832, 3826, 3825, 3791, 3793, 3744, 3747, 3863, 3862, 3884, 3880
* **→ Guard / DLP (Audrey Garcia + Information Protection):** ENT-3882, 3872, 3852, 3851, 3823, 3815
* **→ Confluence / Jira / JSM / Loom (Product teams):** Loom (Kristen Waters) — ENT-3814, 3817-3822 · Confluence — ENT-3861, 3829, 3828, 3846, 3840, 3743, 3859, 3883 · Jira platform — ENT-3827, 3845, 3868 · JSM (Assets / CMDB) — ENT-3873, 3858, 3839, 3783, 3742 · JSM Other — ENT-3857 · Ecosystem Platform — ENT-3854, 3871
* **→ Migration / Other:** ENT-3789 (sandbox separation), ENT-3870 (video embedding for onboarding)

## C — Tickets needing TPM triage attention (no assignee)
...
* Two ticket keys (ENT-351, ENT-293) were not retrievable via the rich `get` call; ENT-293 *is* retrievable via `jira workitem search` (it appears in the open Blocker list).
* The recent-inbox set deliberately **excludes** Loom-only, Confluence-only, and JSM-only product items from CoreEng pillar columns — those are routed but not owned by CoreEng.
