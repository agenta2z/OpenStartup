...
def discover_databricks_token(host: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            return (
                w.config.authenticate().get("Authorization", "").replace("Bearer ", "")
                or None
            )
        except Exception as e:
            _step(
                f"databricks-sdk default auth chain: {type(e).__name__}: {str(e)[:120]}"
            )
    except ImportError:
    ...
    if shutil.which("atlas"):
        ...
        try:
            ...
            if r.returncode == 0 and r.stdout.startswith("eyJ"):
                _ok(
                    f"SLAuth token mints OK (Kerberos alive). "
                ...
                _warn(
                    f"slauth returned rc={r.returncode}: {(r.stderr or r.stdout)[:120]}"
                )
        except Exception as e:
...
def run_metrics_query(
    host: str, token: str, warehouse_id: Optional[str], days: int
) -> int:
    ...
    _step(
        f"Submitting SQL: SELECT ... FROM online_eval_metrics "
    ...
    resp = requests.post(
        ...
        json={
            ...
            "statement": METRICS_QUERY.replace(":start_day", f"'{start_day}'"),
            "wait_timeout": "30s",
            "format": "JSON_ARRAY",
        },
        timeout=60,
