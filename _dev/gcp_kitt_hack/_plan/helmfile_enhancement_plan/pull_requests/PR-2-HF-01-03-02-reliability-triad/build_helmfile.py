...
def main():
    ...
    if src.count(old) != 1:
        sys.exit(
            f"FATAL: server.replicaCount block found {src.count(old)} times, expected 1"
        )
    src = src.replace(old, new, 1)
    ...
    if src.count(old) != 1:
        sys.exit(
            f"FATAL: web.replicaCount block found {src.count(old)} times, expected 1"
        )
    src = src.replace(old, new, 1)
    ...
    if src.count(anchor) != 1:
        sys.exit(
            f"FATAL: server-config-end anchor found {src.count(anchor)} times, expected 1"
        )
    new_anchor = (
        "              maxWorkflowExecutionHistoryCount: 100000\n"
        + nested_role_blocks
        + "        matching:"
    )
    ...
    if src.count(anchor) != 1:
        sys.exit(
            f"FATAL: gossip-hook anchor found {src.count(anchor)} times, expected 1"
        )
