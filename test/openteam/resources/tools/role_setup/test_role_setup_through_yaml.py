"""E2E test for the real /role_setup command path.
...
Usage::
    ...
    python -m test.openteam.resources.tools.role_setup.test_role_setup_through_yaml \
...
def main(role_document, max_facets, max_inner_facets, output_dir, log_level):
    ...
    # =====================================================================
    arguments = {"role_document_path": str(Path(role_document).resolve())}
    ...
    session_context = {
        ...
        "interactive": None,
        "task_id": f"test-role-setup-{timestamp}",
    ...
    click.echo(f"Running /role_setup via execute()...")
    click.echo(f"Role document: {role_document}")
    ...
    report_path = result.context_updates.get("role_setup_report_path")
    ...
    click.echo("")
    click.echo("=" * 60)
    click.echo("Role Setup — Complete (via execute())")
    click.echo("=" * 60)
