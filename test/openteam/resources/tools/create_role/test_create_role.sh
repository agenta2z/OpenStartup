...
# test_create_role.sh
#
# End-to-end live-LLM smoke-test launcher for the create_role tool CLI.
#
# Purpose:
...
#   - Launches the create_role CLI with a user-supplied role description.
#   - Streams output to a timestamped log in the repo root (or override).
#
# Usage:
#   ./test_create_role.sh "hire a machine learning engineer (MLE)"
#   ./test_create_role.sh --max-facets 5 "hire a backend engineer"
#   ./test_create_role.sh --background "hire a UX designer"
#
# Options (before the positional ROLE_DESCRIPTION):
#   --max-facets N             Number of facets to break role into (default 3)
    ...
    case "$1" in
        ...
        *)
            if [ -z "${ROLE_DESCRIPTION}" ]; then
                ...
                die "Multiple positional arguments. Quote the role description."
            fi
            shift
            ;;
    esac
...
if [ -z "${ROLE_DESCRIPTION}" ]; then
    die "Missing role description.

Usage:
  $0 [OPTIONS] \"<role description>\"

Examples:
  $0 \"hire a machine learning engineer (MLE)\"
  $0 --max-facets 5 \"hire a backend engineer\"
  $0 --background \"hire a UX designer\"
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/create_role_${TIMESTAMP}.log"
...
# -----------------------------------------------------------------------------
log_info "Verifying create_role module is importable..."

if ! ${PYTHON_BIN} -c "import openteam.server.resources.tools.create_role" >/dev/null 2>&1; then
    log_error "Cannot import openteam.server.resources.tools.create_role"
    log_error "PYTHONPATH: ${PYTHONPATH}"
    ${PYTHON_BIN} -c "import openteam.server.resources.tools.create_role" 2>&1 | tail -5
...
echo
echo -e "${BOLD}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BOLD}  create_role test launch — ${TIMESTAMP}${NC}"
echo -e "${BOLD}═══════════════════════════════════════════════════════════════${NC}"
echo "  Role description:  ${ROLE_DESCRIPTION}"
...
CMD=(
    "${PYTHON_BIN}"
    -m openteam.server.resources.tools.create_role
...
if [ ${BACKGROUND} -eq 1 ]; then
    ...
    if kill -0 "${PID}" 2>/dev/null; then
        ...
        echo "  ${REPO_ROOT}/_runtime/tasks/create_role/create_role_${TIMESTAMP}_*"
        exit 0
    else
