...
@pytest.mark.skip(
    reason="TIER 3: run only in CI with profile=quick; skipped locally to save time"
)
@pytest.mark.slow
def test_smoke_run_minimal_task_with_quick_profile(tmp_path, monkeypatch):
    ...
    task_file.write_text(
        "Document the architecture of src/openteam/server/main.py", encoding="utf-8"
    )

    target_file = tmp_path / "target.py"
    target_file.write_text(
        "# Sample file to document\ndef example(): pass", encoding="utf-8"
    )
    ...
    cmd = [
        ...
        str(output_dir),
        "--profile",
        "quick",  # Quick profile = minimal LLM calls
        "--timeout",
        "600",  # 10 minutes
    ]
