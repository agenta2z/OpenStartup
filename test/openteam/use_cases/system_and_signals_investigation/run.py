"""Phase 2 prototype: system & operational-signals investigation.

Single-shot specialised script.  Required parameter: ``--codebase``.  Phase-1
codebase docs are auto-discovered from
``~/.ai-employee/projects/<workstream>/artifacts/codebase_documentation/`` (and
fall back to the legacy in-package ``_runtime/`` locations for older runs).

Outputs land under ``~/.ai-employee/projects/<workstream>/`` with:

* ``artifacts/system_and_signals_documentation/`` — durable, latest-only docs
* ``_runtime/system_and_signals_investigation/run_<ts>_<uuid>/`` — debug

Usage:
    python -m openteam.use_cases.system_and_signals_investigation.run \\
...
PROMPT_PATH = HERE / "prompts" / "investigate_system_and_signals.md"
PHASE_NAME = "signals"
SCRIPT_DIR_NAME = "system_and_signals_investigation"
...
def _render_prompt(target_path: Path, codebase_docs: Path | None, output_docs_dir: Path) -> str:
    ...
    docs_placeholder = (
        str(codebase_docs)
        if codebase_docs and codebase_docs.exists()
        else "(no Phase-1 docs found — proceed with codebase-only inference)"
    )
    return (
        template.replace("{{TARGET_PATH}}", str(target_path))
        .replace("{{CODEBASE_DOCS_DIR}}", docs_placeholder)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional explicit path to Phase-1 docs (auto-discovered otherwise).",
    )
    parser.add_argument(
        "--runtime-root",
        type=Path,
    ...
    if args.codebase_docs:
        phase1_docs = args.codebase_docs.expanduser().resolve()
    else:
        phase1_docs = autodiscover_phase_artifacts(
            codebase=codebase,
            phase_name="codebase",
