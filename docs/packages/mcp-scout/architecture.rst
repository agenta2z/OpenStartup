...
This page describes the runtime architecture of ``mcp-scout``.
...
* ``mcp_scout/__init__.py`` -- public re-exports.
* ``mcp_scout/server`` or ``mcp_scout/app`` -- transport
  surface (FastAPI app, MCP server, or CLI dispatcher).
* ``mcp_scout/adapters`` -- thin clients around third-party SDKs.
* ``mcp_scout/models`` -- pydantic / dataclass schemas used by the
  public API.
* ``mcp_scout/config`` -- environment-driven settings.
