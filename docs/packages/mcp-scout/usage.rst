...
checkout::
    ...
    pip install -e packages/mcp-scout
...
The package ships a ``mcp-scout`` console script. Show available subcommands::
...
.. code-block:: python
    ...
    from mcp_scout import build_default_client
...
End-to-end recipes live under ``examples/mcp-scout/`` in the monorepo and
...
* **Missing dependencies** -- verify the extras for the package are
  installed (``pip install -e 'packages/mcp-scout[all]'``).
