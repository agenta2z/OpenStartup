...
``mcp-atlassian-exp`` is the **INTEGRATION (MCP)** package responsible for delivering
...
* **Application developers** integrating ``mcp-atlassian-exp`` into a workflow.
...
* Programmatic API exposed by the ``mcp_atlassian_exp`` Python module.
