...
checkout::
    ...
    pip install -e packages/code-conseil
...
The package ships a ``code-conseil`` console script. Show available subcommands::
...
.. code-block:: python
    ...
    from code_conseil import build_default_client
...
End-to-end recipes live under ``examples/code-conseil/`` in the monorepo and
...
* **Missing dependencies** -- verify the extras for the package are
  installed (``pip install -e 'packages/code-conseil[all]'``).
