...
checkout::
    ...
    pip install -e packages/code-nautilus
...
* Package-specific overrides documented in :doc:`api` under
  ``code_nautilus.config``.

Command-line
----------------------------------------

The package ships a ``code-nautilus`` console script. Show available subcommands::
...
.. code-block:: python
    ...
    from code_nautilus import build_default_client
...
End-to-end recipes live under ``examples/code-nautilus/`` in the monorepo and
...
* **Missing dependencies** -- verify the extras for the package are
  installed (``pip install -e 'packages/code-nautilus[all]'``).
