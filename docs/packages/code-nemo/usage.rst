...
checkout::
    ...
    pip install -e packages/code-nemo
...
The package ships a ``code-nemo`` console script. Show available subcommands::
...
.. code-block:: python
    ...
    from code_nemo import build_default_client
...
End-to-end recipes live under ``examples/code-nemo/`` in the monorepo and
...
* **Missing dependencies** -- verify the extras for the package are
  installed (``pip install -e 'packages/code-nemo[all]'``).
