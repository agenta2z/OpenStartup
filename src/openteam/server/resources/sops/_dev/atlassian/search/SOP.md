...
**Poll**: `atlas ml workflow run-status -w <ml_studio_run_id> -e prod --simple` (returns `"status":"RUNNING|SUCCEEDED|FAILED"`).

## Phase 2 -- Filtered training (the real model):
[__depends on__ Phase 0; may parallel with Phase 1]
...
**Wait for SUCCESS before Phase 3.** Do NOT proceed to packaging if Phase 2 fails or aborts — packaging would consume a stale model.

## Phase 3 -- Tarot V2 packaging:
[__depends on__ Phase 2 succeeded]
...
1. **Single-region eval mistake**: First eval batch ran `ab41e4d7` (us_west_2) 4 times. Caught only after critical-thinking double-check. Mitigation: this SOP now lists the 4 region-specific blueprint IDs explicitly.

2. **MLflow URI placeholder**: The `lm_uri` field in the Tarot V3 YAML had to be filed as `PLACEHOLDER_MLFLOW_RUN_ID` because experiment `3747328226380826` is in a workspace context inaccessible from `databricks://ml-ugc-prod`. Reviewer must resolve manually.

3. **MRR extraction**: Cannot be automated without additional SSAM. Launch doc was published with "MRR pending" entries and a footer comment explaining the resolution path.

4. **Slack send**: No Slack MCP / webhook in Rovo Dev session. Orchestrator prints copy-pasteable message; user sends.

5. **Coordination risk**: If a recent refresh exists (e.g., 2026-06-02 v35 happened the day after 2026-06-01 v34 by hchang3), include a back-out offer in the Slack message to avoid stepping on rollouts in flight.

6. **Watcher script awk fragility**: The first `run_chain_watcher.sh` failed to extract Phase 3 run-id because it expected `Run ID:` but `atlas ml workflow run` outputs `ML Studio Run ID:`. Always validate non-empty rid before entering a poll loop.
